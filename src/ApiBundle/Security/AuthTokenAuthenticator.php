<?php
# src/ApiBundle/Security/AuthTokenAuthenticator.php
...
{
    /**
    * Durée de validité du token en secondes, 15min
    */
    const TOKEN_VALIDITY_DURATION = 15 * 60;
    ...
    public function createToken(Request $request, $providerKey)
    {
        ...
        if ( ($request->getMethod() === "POST" || $request->getMethod() === "OPTIONS") && $this->httpUtils->checkRequestPath($request, $targetUrl)) {
            return;
        }
        ...
        if (!$authTokenHeader) {
            throw new UnauthorizedHttpException('No token given or token is wrong.','No token given or token is wrong.');
        }
